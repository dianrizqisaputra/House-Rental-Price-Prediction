# -*- coding: utf-8 -*-
"""Predictive Analytics - Prediksi Harga Sewa Rumah (Dian Rizqi Saputra).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1vKQR_gTusA_wsraVHlUfUWU3oFH7wXjz
...
linkedin : https://www.linkedin.com/in/dianrizqisaputra/
...
This dataset was taken from: https://www.kaggle.com/datasets/iamsouravbanerjee/house-rent-prediction-dataset
"""

!gdown --id "1Uy6b7iIFghw1WN-cjibw6ukc_cwox2wN"
...
# Fitur Point of Contract dan Posted On does not affect the model rental price so it will be dropped
...
# The Area Type feature only has 2 Built Area samples so these 2 samples will be deleted
...
# 100 sqft for 1 BHK is not unusual so let's assume the threshold limit is 300 sqft/bhk

df[(df.Size/df.BHK) < 300].head()

df.shape

# Removed outlier size per BHK
...
# Remove price per sqft outliers with mean and one standard deviation
...
# Detect bathroom outliers
# 2 BHK with 4 bathrooms is not uncommon so let's just say the bathroom limit should not exceed the number of BHKs + 2
...
# Removed the price per sqft feature because it is no longer used
df3 = df2.drop(['Price_per_sqft'], axis = 'columns')

# See the correlation between numeric features and target features (price)
...
# See the correlation between categorical features and target features (price)
